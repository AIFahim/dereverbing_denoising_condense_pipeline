...
def eco_cancel_gpu(input_path: str) -> None:
    print("Echo cancelling . . . ")
    
    # Define paths
    checkpoint_file = '/home/asif/Noise_Filtering_Works/dereverb_denoise_condenser/unet_state_dict.pth'
    out_path = Path('/home/asif/Noise_Filtering_Works/dereverb_denoise_condenser/ttsout') / Path(input_path).name
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # Load model and move to GPU
    
    # model_unet = torch.jit.load(checkpoint_file)
    # checkpoint_file = 'unet_state_dict.pth'
    model_unet = UNetRev(n_channels=1, bilinear=False, confine = False)
    model_unet.load_state_dict(torch.load(checkpoint_file,map_location=device))
    ...
    spec = spectrogram(SPEECH_WAVEFORM.to(device))
    spec_db = librosa.power_to_db(spec[0].cpu().numpy())

    # Perform echo cancellation
    with torch.no_grad():
        derev = model_unet(spec.to(device))

    derev_db = derev.cpu().numpy()[0, 0, :, :]
    reconstructed_waveform = GriffinLim(
        n_fft=2048,
        win_length=None,
        hop_length=512,
    ).to(device)(torch.from_numpy(librosa.db_to_power(derev_db, ref=1.0)).unsqueeze(0))
    
    # Save output
    torchaudio.save(out_path, reconstructed_waveform.cpu(), sample_rate=SAMPLE_RATE)
